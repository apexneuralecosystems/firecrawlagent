...
### 4. Environment Setup
Copy the `.envexample` file to `.env` and fill in your API keys:
```bash
cp .envexample .env
```
...
This includes automatic setup checks and helpful error messages. See [EXECUTION_GUIDE.md](EXECUTION_GUIDE.md) for more options.
...
### Option 4: Setup Verification First
